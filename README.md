...
┌───────────────────────▼─────────────────────────────────┐
│                   Agent 逻辑层                           │
│  固定流水线 → 编排 Skill → 聚合结果 → 生成报告          │
│  核心逻辑：IOC 去重合并 / 分支早退 / Skill 降级          │
└───────────────────────┬─────────────────────────────────┘
                        │
┌───────────────────────▼─────────────────────────────────┐
│                   Skill 工具层（可插拔）                  │
...
| **Scheduler**    | 任务执行         | 执行单个 Skill 并记录调用历史；失败时记录错误并上抛异常（降级由各 Skill 内部 try/except 兜底，非 Scheduler 切换备用 Skill） |
...
每步执行后会检查结果，但仅做**早退短路**（正文为空 / 未提取到 IOC / 全被白名单过滤 / 无情报 key 时直接结束），不动态选择或重排后续步骤。所谓“动态降级”实为各 Skill**内部**的兜底（LLM 无 key 走本地启发式、threat_intel 无 key 走 mock、各自 try/except），非流水线层重规划。真正的动态编排（如 LangGraph）见路线图，尚未落地。
...
- **上下文提取**：每个 IOC 自动附带前后各 2 句作为上下文；若切分结果过长（> `MAX_CONTEXT_CHARS`=800 字符，常见于无标点的纯哈希列表），回退为按行取前后各 `LINE_CONTEXT_WINDOW`=3 行
...
- **可扩展**：支持通过 `data/custom_whitelist.txt` 添加自定义**域名**白名单（IP 段与文件路径白名单暂为内置硬编码，不走自定义文件）
...
- **Prompt 策略**：
  - Zero-Shot 提示词（`DEFAULT_PROMPT_TEMPLATE` 给出分析原则，无样本示例）
  - 强制结合上下文语义判断（“攻击手法”章节 vs “参考资料”章节）
  - 输出 JSON 结构化结果（`malicious` / `label` / `reason`）
- **本地兜底**：无 API Key（或调用失败/JSON 解析失败）时自动切换为基于关键词的启发式规则分析
...
- **默认模型**：`deepseek-v4-flash`（可通过 `LLM_MODEL` 环境变量切换；国内直连、成本低）
- **Prompt 模板**：内联在 `skills/llm_analyzer/main.py` 的 `DEFAULT_PROMPT_TEMPLATE`（`prompts/ioc_classify.j2` 为历史预留，当前未接线）
...
| 网页抓取失败        | 抛出异常上浮；批量模式下记为「访问失败」不中断整批 |
